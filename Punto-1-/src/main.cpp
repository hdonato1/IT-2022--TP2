...
int LED_BUILTIN = 2;                           //Definimos el pin de salida - GPIO2 / D4
int estado = !LOW;                         //Definimos la variable que va a recoger el estado del LED
void setup() {
  Serial.begin(9600);
  pinMode(LED_BUILTIN, OUTPUT);                //Inicializamos el GPIO2 como salida
  digitalWrite(LED_BUILTIN, !LOW);              //Dejamos inicialmente el GPIO2 apagado
...
void loop() 
{
  ...
  client.print("<h1>El LED está ahora:</h1>");                 
  if(estado == !HIGH) {
    client.print("<h1>ENCENDIDO</h1>");  
  } else {
